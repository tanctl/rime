...
use rime_core::{
    ...
    DetectionKeyConfig, FullMemoConfig, Network, PirConfig, PirServerConfig, PrivacyConfig,
    SyncMode, Wallet, WalletMetadata, WalletSeed, WalletStore,
};
...
struct SyncArgs {
    ...
    #[arg(
        long = "darkside",
        help = "Enable synthetic/darkside mode (required for detection-key mode)"
    )]
    darkside: bool,
    #[arg(
        long = "detection-key",
        value_name = "hex",
        help = "Hex-encoded detection key used with detection-key sync mode"
    )]
    detection_key: Option<String>,
    #[arg(
        long = "tag-bits",
        value_name = "n",
        help = "Number of leading tag bits to match (detection-key mode)"
    )]
    tag_bits: Option<u8>,
...
enum CliSyncMode {
    ...
    Pir,
    /// simulation-only detection key filtering
    DetectionKey,
}
...
impl From<CliSyncMode> for SyncMode {
    fn from(value: CliSyncMode) -> Self {
        match value {
            ...
            CliSyncMode::DetectionKey => SyncMode::DetectionKey,
        }
    }
}
...
impl SyncArgs {
    fn privacy_config(&self) -> Result<PrivacyConfig, CliConfigError> {
        ...
        if !matches!(sync_mode, SyncMode::DetectionKey) {
            self.ensure_no_detection_key_flags()?;
        }
        ...
        match sync_mode {
            ...
            SyncMode::DetectionKey => {
                let detection_key = self.detection_key.clone().ok_or_else(|| {
                    CliConfigError::new("--detection-key is required for detection-key sync mode")
                })?;
                let tag_bits = self.tag_bits.unwrap_or(16);
                config.detection_key = Some(DetectionKeyConfig {
                    key: detection_key,
                    darkside_mode: self.darkside,
                    tag_bits,
                });
            }
        }
    ...
    fn ensure_no_detection_key_flags(&self) -> Result<(), CliConfigError> {
        if self.detection_key.is_some() || self.tag_bits.is_some() {
            return Err(CliConfigError::new(
                "--detection-key/--tag-bits require --sync-mode detection-key",
            ));
        }
        Ok(())
    }
    ...
    match sync_mode {
        ...
        SyncMode::DetectionKey => {
            return Err(boxed(io::Error::other(
                "DetectionKey mode is not yet implemented.",
            )))
        }
    }
...
mod tests {
    ...
    #[test]
    fn detection_mode_requires_darkside() {
        ...
        let err = args.privacy_config().unwrap_err();
        assert!(err.to_string().contains("darkside"));
    }

    #[test]
    fn detection_mode_validates_hex() {
        let args = parse_sync([
            "rime-cli",
            "sync",
            "--sync-mode",
